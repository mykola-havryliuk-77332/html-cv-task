...
form.addEventListener('submit', function(event) {
    ...
    if (isValid) {
        // Zapis danych poza przeglądarką (Firebase Realtime Database)
        
        // Tworzymy obiekt z danymi formularza
        const daneFormularza = {
            imie: imie,
            nazwisko: nazwisko,
            email: email,
            wiadomosc: wiadomosc,
            data_wyslania: new Date().toLocaleString()
        };

      
        const firebaseUrl = 'https://cv-backend-cc21f-default-rtdb.europe-west1.firebasedatabase.app/wiadomosci.json';

        // Wysyłamy dane metodą POST za pomocą fetch()
        fetch(firebaseUrl, {
            method: 'POST',
            headers: {
        ...
        .then(response => {
            if (response.ok) {
                // Potwierdzenie poprawnego wysłania
                alert('Sukces! Twoja wiadomość została zapisana w bazie Firebase.');
                form.reset(); 
            } else {
